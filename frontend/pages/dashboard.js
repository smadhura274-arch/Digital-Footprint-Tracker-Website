...
router.get('/stats', protect, async (req, res) => {
    try {
        ...
        const handles = (latestScan && latestScan.socialHandles) ? latestScan.socialHandles : {};
...
// @route   GET /api/dashboard/report/generate
// @desc    Trigger a report data refresh
// @access  Private
router.get('/report/generate', protect, async (req, res) => {
    try {
        // In a production environment, this might trigger an aggregation job.
        // Here we return success to allow the frontend to refresh its local history.
        res.json({ success: true, message: 'Report analysis refreshed' });
    } catch (error) {
